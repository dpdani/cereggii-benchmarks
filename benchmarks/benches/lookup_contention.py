...
def lookup_contention(dict_factory, skew, threads):
    ...
    if issubclass(dict_factory, Growt):
        return utils.BenchmarkPlan(
            name="lookup_contention",
            threads=[],
            barrier_1=None,  # noqa
            barrier_2=None,  # noqa
            params=params,
            debug=lambda: {},
            wall_clock_time=Growt().lookup_contention(
                threads=threads,
                keys_count=keys_count,
                min_size=keys_count,
                skew=skew,
    ...
    assert len(keys) == keys_count

    d = dict_factory({
        k: None
        for k in range(keys_count + 1)
    })
