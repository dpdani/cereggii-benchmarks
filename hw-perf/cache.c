// copied from Hennessy, Patterson, Computer Architecture, A Quantitative Approach

//#include "stdafx.h"
#include <stdio.h>
#include <time.h>
#define ARRAY_MIN (8192) /* 1/4 smallest cache */
#define ARRAY_MAX (3145728) /* 1/4 largest cache */
...
int label(int i) {/* generate text labels */
  if (i<1e3) printf("%3dB, ",i);
  else if (i<1e6) printf("%3dK, ",i/1024);
  else if (i<1e9) printf("%3dM, ",i/1048576);
  else printf("%3dG, ",i/1073741824);
  return 0;
}
...
for (csize=ARRAY_MIN; csize <= ARRAY_MAX; csize=csize*2) {
  ...
  for (stride=1; stride <= csize/2; stride=stride*2) {
    ...
    } while ((sec1 - sec0) < 10.0); /* collect 20 seconds */
    sec = sec1 - sec0;
